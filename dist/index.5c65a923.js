...
},{}],"a0wRA":[function(require,module,exports) {
...
const modal = new _bootstrapNativeDefault.default.Modal('#exampleModal');
let promptCounter = 0;
...
function openModal() {
    if (promptCounter === MAX_PROMPT_ATTEMPTS || hasSubscribe) return;
    setTimeout(()=>{
...
},{"bootstrap.native":"sarx1","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"sarx1":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
function getElementTransitionDuration(element) {
    ...
    const duration = supportTransition && propertyValue && propertyValue !== 'none' ? parseFloat(durationValue) * durationScale : 0;
...
function bootstrapCustomEvent(namespacedEventType, eventProperties) {
    const OriginalCustomEvent = new CustomEvent(namespacedEventType, {
        cancelable: true
    });
    if (eventProperties instanceof Object) Object.keys(eventProperties).forEach((key)=>{
...
function normalizeOptions(element, defaultOps, inputOps, ns) {
    ...
    Object.keys(data).forEach((k)=>{
        const key = k.includes(ns) ? k.replace(ns, '').replace(/[A-Z]/, (match)=>match.toLowerCase()
        ) : k;
        dataOps[key] = normalizeValue(data[k]);
    });
    Object.keys(inputOps).forEach((k)=>{
        inputOps[k] = normalizeValue(inputOps[k]);
    });
    Object.keys(defaultOps).forEach((k)=>{
        if (k in inputOps) normalOps[k] = inputOps[k];
        else if (k in dataOps) normalOps[k] = dataOps[k];
        else normalOps[k] = defaultOps[k];
    });
    return normalOps;
}
...
function carouselTransitionEndHandler(self) {
    const { index , direction , element , slides , options , isAnimating ,  } = self;
    ...
    if (isAnimating && element[carouselComponent]) {
        ...
        if (!document.hidden && options.interval && !hasClass(element, pausedClass)) self.cycle();
    }
}
...
function carouselIndicatorHandler(e) {
    ...
    if (target && !hasClass(target, activeClass) // event target is not active
     && newIndex) self.to(+newIndex); // do the slide
}
function carouselControlsHandler(e) {
    ...
    const { controls  } = self;
    if (controls[1] && that === controls[1]) self.next();
    else if (controls[1] && that === controls[0]) self.prev();
}
function carouselKeyHandler({ which  }) {
    const [element] = Array.from(document.querySelectorAll(carouselSelector)).filter((x)=>isElementInScrollRange(x)
    );
    if (!element) return;
...
function carouselTouchMoveHandler(e) {
    ...
    if (type === 'touchmove' && changedTouches.length > 1) e.preventDefault();
}
function carouselTouchEndHandler(e) {
    ...
    if (self.isTouch) {
        ...
        if ((!element.contains(e.target) || !element.contains(e.relatedTarget)) && Math.abs(startX - endX) < 75) // when the above conditions are satisfied, no need to continue
        return;
         // OR determine next index to slide to
        if (currentX < startX) self.index += 1;
        else if (currentX > startX) self.index -= 1;
        self.isTouch = false;
...
function toggleCarouselHandlers(self, add) {
    const { element , options , slides , controls , indicator ,  } = self;
    ...
    if (touch && slides.length > 1) element[action]('touchstart', carouselTouchDownHandler, passiveHandler);
...
function getActiveIndex(self) {
    ...
    return Array.from(slides).indexOf(element.getElementsByClassName(`${carouselItem} ${activeClass}`)[0]) || 0;
}
// CAROUSEL DEFINITION
// ===================
class Carousel extends BaseComponent {
    constructor(target4, config1){
        ...
        self.options.interval = options.interval === true ? defaultCarouselOptions.interval : options.interval;
    ...
    to(idx1) {
        ...
        if (activeItem < next || activeItem === 0 && next === slides.length - 1) self.direction = 'left'; // next
        else if (activeItem > next || activeItem === slides.length - 1 && next === 0) self.direction = 'right'; // prev
        ...
        if (getElementTransitionDuration(slides[next]) && hasClass(element, 'slide')) {
            ...
            setTimeout(()=>{
                self.isAnimating = false;
                // check for element, might have been disposed
                if (element && options.interval && !hasClass(element, pausedClass)) self.cycle();
                element.dispatchEvent(carouselSlidEvent);
            }, 100);
        }
    }
...
function getTargetElement(element) {
    return queryElement(element.getAttribute(dataBsTarget) || element.getAttribute('href')) || element.closest(element.getAttribute(dataBsParent)) || queryElement(element.getAttribute(dataBsContainer));
}
...
function toggleCollapseHandler(self, add) {
    const action = add ? addEventListener : removeEventListener;
    const { triggers  } = self;
    if (triggers.length) triggers.forEach((btn)=>btn[action]('click', collapseClickHandler)
    );
}
...
class Collapse extends BaseComponent {
    constructor(target5, config2){
        ...
        self.triggers = Array.from(document.querySelectorAll(collapseToggleSelector)).filter((btn)=>getTargetElement(btn) === element
        );
    ...
    show() {
        const self = this;
        const { element , parent , triggers , isAnimating ,  } = self;
        let activeCollapse;
        let activeCollapseInstance;
        if (parent) {
            activeCollapse = Array.from(parent.querySelectorAll(`.${collapseString}.${showClass}`)).find((i)=>i[collapseComponent]
            );
...
function isEmptyAnchor(elem) {
    ...
    return elem && (elem.hasAttribute('href') && elem.href.slice(-1) === '#' || parentAnchor && parentAnchor.hasAttribute('href') && parentAnchor.href.slice(-1) === '#');
}
...
function styleDropdown(self, show) {
    const { element , menu , originalClass , menuEnd , options ,  } = self;
    ...
    if (horizontalClass.includes(positionClass) && bottomExceed) positionClass = dropupString;
    if (positionClass === dropstartString && leftFullExceed && !bottomExceed) positionClass = dropendString;
    if (positionClass === dropendString && rightFullExceed && !bottomExceed) positionClass = dropstartString;
    ...
    menu.style.margin = `${dropdownMargin.map((x)=>x ? `${x}px` : x
    ).join(' ')}`;
    ...
    if (!hasClass(parent, positionClass)) parent.className = parent.className.replace(dropdownRegex, positionClass);
    ...
    if (verticalClass.includes(positionClass)) {
        ...
        if (hasClass(menu, dropdownMenuEndClass)) Object.keys(dropdownPosition.menuEnd).forEach((p)=>{
...
function getCurrentOpenDropdown() {
    const currentParent = dropdownMenuClasses.concat('btn-group').map((c)=>document.getElementsByClassName(`${c} ${showClass}`)
    ).find((x)=>x.length
    );
    if (currentParent && currentParent.length) return Array.from(currentParent[0].children).find((x)=>x.hasAttribute(dataBsToggle)
    );
    return null;
}
...
function dropdownDismissHandler(e) {
    ...
    const isForm = parent && parent.contains(target) && (target.tagName === 'form' || target.closest('form') !== null);
    if (type === 'click' && isEmptyAnchor(target)) e.preventDefault();
    if (type === 'focus' && (target === element || target === menu || menu.contains(target))) return;
    if (isForm || hasData) ;
    else if (self) self.hide(element);
}
...
function dropdownKeyHandler({ which  }) {
    ...
    if (isMenuItem) {
        if (isSameElement) idx = 0;
        else if (which === 38) idx = idx > 1 ? idx - 1 : 0;
        else if (which === 40) idx = idx < menuItems.length - 1 ? idx + 1 : idx;
        if (menuItems[idx]) setFocus(menuItems[idx]);
    }
    if ((menuItems.length && isMenuItem || !menuItems.length && (isInsideMenu || isSameElement) || !isInsideMenu) && open && which === 27 // menu must be open
    ) self.toggle();
}
...
const fixedItems = Array.from(document.getElementsByClassName(fixedTopClass)).concat(Array.from(document.getElementsByClassName(fixedBottomClass))).concat(Array.from(document.getElementsByClassName(stickyTopClass))).concat(Array.from(document.getElementsByClassName('is-fixed')));
...
function isVisible(element) {
    return getComputedStyle(element).visibility !== 'hidden' && element.offsetParent !== null;
}
...
function setModalScrollbar(self) {
    ...
    const bd = document.body;
    const html = document.documentElement;
    const bodyOverflow = html.clientHeight !== html.scrollHeight || bd.clientHeight !== bd.scrollHeight;
    const modalOverflow = element.clientHeight !== element.scrollHeight;
    if (!modalOverflow && scrollbarWidth) element.style.paddingRight = `${scrollbarWidth}px`;
...
function toggleModalHandler(self, add) {
    const action = add ? addEventListener : removeEventListener;
    const { triggers  } = self;
    if (triggers.length) triggers.forEach((btn)=>btn[action]('click', modalClickHandler)
    );
}
...
function beforeModalShow(self) {
    ...
    if (!getCurrentOpen()) document.body.style.overflow = 'hidden';
    addClass(element, showClass);
...
function beforeModalHide(self, force) {
    const { element , options , relatedTarget , hasFade ,  } = self;
    element.style.display = '';
    // force can also be the transitionEvent object, we wanna make sure it's not
    // call is not forced and overlay is visible
    if (options.backdrop && !force && hasFade && hasClass(overlay, showClass) && !getCurrentOpen()) {
        hideOverlay();
...
class Modal extends BaseComponent {
    constructor(target7, config4){
        ...
        self.triggers = Array.from(document.querySelectorAll(modalToggleSelector)).filter((btn)=>getTargetElement(btn) === element
        );
    ...
    show() {
        const self = this;
        const { element , options , isAnimating , hasFade , relatedTarget ,  } = self;
        const { backdrop  } = options;
        let overlayDelay = 0;
        ...
        if (currentOpen && currentOpen !== element) {
            const that = currentOpen[modalComponent] ? currentOpen[modalComponent] : currentOpen.Offcanvas;
            that.hide();
        }
        self.isAnimating = true;
        if (backdrop) {
            if (!currentOpen && !hasClass(overlay, showClass)) appendOverlay(hasFade, 1);
            else toggleOverlayType(1);
            overlayDelay = getElementTransitionDuration(overlay);
            if (!hasClass(overlay, showClass)) showOverlay();
            setTimeout(()=>beforeModalShow(self)
            , overlayDelay);
        } else {
    ...
    hide(force) {
        const self = this;
        const { element , isAnimating , hasFade , relatedTarget ,  } = self;
        if (!hasClass(element, showClass) && !isAnimating) return;
        ...
        if (hasFade && force !== 1) emulateTransitionEnd(element, ()=>beforeModalHide(self)
        );
        else beforeModalHide(self, force);
    }
    update() {
        const self = this;
...
function offcanvasDismissHandler(e) {
    ...
    if (trigger && trigger.tagName === 'A') e.preventDefault();
    if (!element.contains(target) && options.backdrop && (!trigger || trigger && !triggers.includes(trigger)) || offCanvasDismiss && offCanvasDismiss.contains(target)) {
...
function showOffcanvasComplete(self) {
    ...
    self.isAnimating = false;
    if (triggers.length) triggers.forEach((btn)=>btn.setAttribute(ariaExpanded, true)
    );
...
function hideOffcanvasComplete(self) {
    const { element , options , relatedTarget , triggers ,  } = self;
    const currentOpen = getCurrentOpen();
    element.setAttribute(ariaHidden, true);
    element.removeAttribute(ariaModal);
...
class Offcanvas extends BaseComponent {
    constructor(target8, config5){
        ...
        self.triggers = Array.from(document.querySelectorAll(offcanvasToggleSelector)).filter((btn)=>getTargetElement(btn) === element
        );
    ...
    show() {
        ...
        const { element , options , isAnimating , relatedTarget ,  } = self;
        let overlayDelay = 0;
        if (hasClass(element, showClass) || isAnimating) return;
        ...
        const currentOpen = getCurrentOpen();
        if (currentOpen && currentOpen !== element) {
            const that = currentOpen[offcanvasComponent] ? currentOpen[offcanvasComponent] : currentOpen.Modal;
            that.hide();
        }
        self.isAnimating = true;
        if (options.backdrop) {
            if (!currentOpen) appendOverlay(1);
            else toggleOverlayType();
            overlayDelay = getElementTransitionDuration(overlay);
            if (!hasClass(overlay, showClass)) showOverlay();
            setTimeout(()=>beforeOffcanvasShow(self)
            , overlayDelay);
        } else {
...
function styleTip(self, e) {
    ...
    const { element , options , arrow , positions ,  } = self;
    ...
    topExceed = [
        'left',
        'right'
    ].includes(placement) ? targetRect.top + elemDimensions.h / 2 - tipDimensions.h / 2 < 0 : topExceed;
    bottomExceed = [
        'left',
        'right'
    ].includes(placement) ? targetRect.top + tipDimensions.h / 2 + elemDimensions.h / 2 >= windowHeight : bottomExceed;
    leftExceed = [
        'top',
        'bottom'
    ].includes(placement) ? targetRect.left + elemDimensions.w / 2 - tipDimensions.w / 2 < leftBoundry : leftExceed;
    rightExceed = [
        'top',
        'bottom'
    ].includes(placement) ? targetRect.left + tipDimensions.w / 2 + elemDimensions.w / 2 >= rightBoundry : rightExceed;
    // recompute placement
    // first, when both left and right limits are exceeded, we fall back to top|bottom
    placement = [
        'left',
        'right'
    ].includes(placement) && leftExceed && rightExceed ? 'top' : placement;
    placement = placement === 'top' && topExceed ? 'bottom' : placement;
    placement = placement === 'bottom' && bottomExceed ? 'top' : placement;
    placement = placement === 'left' && leftExceed ? 'right' : placement;
    placement = placement === 'right' && rightExceed ? 'left' : placement;
    // update tooltip/popover class
    if (!tip.className.includes(placement)) tip.className = tip.className.replace(tipClasses, tipClassPositions[placement]);
    ...
    if ([
        ...
        if (placement === 'left') leftPosition = left + scroll.x - tipDimensions.w - (isPopover ? arrowWidth : 0);
        else leftPosition = left + scroll.x + elemDimensions.w + (isPopover ? arrowWidth : 0);
        ...
        if (e && isMedia(element)) {
            const eX = !containerIsRelative ? e.pageX : e.layerX + (absoluteTarget ? element.offsetLeft : 0);
            const eY = !containerIsRelative ? e.pageY : e.layerY + (absoluteTarget ? element.offsetTop : 0);
            if (placement === 'top') topPosition = eY - tipDimensions.h - (isPopover ? arrowWidth : arrowHeight);
            else topPosition = eY + arrowHeight;
            // adjust left | right and also the arrow
            if (e.clientX - tipDimensions.w / 2 < leftBoundry) {
                leftPosition = 0;
                arrowLeft = eX - arrowAdjust;
            } else if (e.clientX + tipDimensions.w * 0.51 >= rightBoundry) {
                leftPosition = 'auto';
                rightPosition = 0;
            ...
            if (placement === 'top') topPosition = top + scroll.y - tipDimensions.h - (isPopover ? arrowHeight : 0);
            else topPosition = top + scroll.y + elemDimensions.h + (isPopover ? arrowHeight : 0);
            // adjust left | right and also the arrow
            if (leftExceed) {
                leftPosition = 0;
...
function setHtml(element, content, sanitizeFn) {
    ...
    if (typeof content === 'object') element.append(content);
    else {
...
const isIphone = navigator.userAgentData ? navigator.userAgentData.brands.some((x)=>appleBrands.test(x.brand)
...
function popoverTouchHandler({ target  }) {
    ...
    if (popover && popover.contains(target) || target === element // OR touch target is element
...
function createPopover(self) {
    ...
    const { animation , customClass , sanitizeFn , placement , dismissible ,  } = options;
    ...
    // load template
    let popoverTemplate;
    if (typeof template === 'object') popoverTemplate = template;
    else {
    ...
    if (customClass && !hasClass(popover, customClass)) addClass(popover, customClass);
...
function togglePopoverHandlers(self, add) {
    ...
    } else if (trigger === 'click') element[action](trigger, self.toggle);
...
class Popover extends BaseComponent {
    constructor(target9, config6){
        ...
        self.options.container = isMedia(element) ? popoverDefaultOptions.container : queryElement(options.container);
    ...
    disable() {
        ...
        if (enabled) {
            if (isVisibleTip(popover, options.container) && options.animation) {
                self.hide();
                setTimeout(()=>togglePopoverHandlers(self)
                , getElementTransitionDuration(popover) + options.delay + 17);
            } else togglePopoverHandlers(self);
            self.enabled = !enabled;
        }
    }
    toggleEnabled() {
...
function updateSpyTargets(self) {
    const { target , scrollTarget , isWindow , options , itemsLength , scrollHeight ,  } = self;
    const { offset  } = options;
...
function activate(self, item) {
    ...
    parents.forEach((menuItem)=>{
        ...
        if (parentLink && !hasClass(parentLink, activeClass)) addClass(parentLink, activeClass);
    });
    // update relatedTarget and dispatch
...
class ScrollSpy extends BaseComponent {
    ...
    refresh() {
        ...
        if (scrollTop >= maxScroll) {
            ...
            if (activeItem !== newActiveItem) activate(self, newActiveItem);
            return;
        }
        const { offsets  } = self;
...
function triggerTabShow(self) {
    const { tabContent , nav  } = self;
    if (tabContent) {
        if (tabEqualContents) triggerTabEnd(self);
        else setTimeout(()=>{
...
function triggerTabHide(self) {
    ...
    if (hasClass(nextTabContent, fadeClass)) setTimeout(()=>{
...
function getActiveTab({ nav  }) {
    ...
    if (activeTabs.length === 1 && !dropdownMenuClasses.some((c)=>hasClass(activeTabs[0].parentNode, c)
    )) [activeTab] = activeTabs;
    else if (activeTabs.length > 1) activeTab = activeTabs[activeTabs.length - 1];
    return activeTab;
}
...
class Tab extends BaseComponent {
    ...
    show() {
        ...
        if (!hasClass(nextTab, activeClass)) {
            ...
            activeTabContent = getActiveTabContent({
                nav
            });
            // update relatedTarget and dispatch
...
function toggleToastHandler(self, add) {
    const action = add ? addEventListener : removeEventListener;
    if (self.dismiss) self.dismiss[action]('click', self.hide);
}
...
class Toast extends BaseComponent {
    ...
    show() {
        ...
        if (element && !hasClass(element, showClass)) {
            ...
            clearTimeout(self.timer);
            self.timer = setTimeout(()=>showToast(self)
            , 10);
        }
    }
    hide(noTimer) {
...
function createTooltip(self) {
    ...
    const { title , template , customClass , animation , placement , sanitizeFn ,  } = options;
    const placementClass = `bs-${tooltipString}-${tipClassPositions[placement]}`;
    if (!title) return;
    // load template
    let tooltipTemplate;
    if (typeof template === 'object') tooltipTemplate = template;
    else {
        const htmlMarkup = document.createElement('div');
        setHtml(htmlMarkup, template, sanitizeFn);
...
function toggleTooltipTitle(self, content) {
    ...
    const { element  } = self;
    element.setAttribute(titleAtt[content ? 0 : 1], content || element.getAttribute(titleAtt[0]));
...
class Tooltip extends BaseComponent {
    constructor(target, config9){
        ...
        const { options  } = self;
        // media elements only work with body as a container
        self.options.container = isMedia(element) ? tooltipDefaultOptions.container : queryElement(options.container);
    ...
    disable() {
        ...
        if (enabled) {
            if (!isVisibleTip(tooltip, options.container) && options.animation) {
                self.hide();
                setTimeout(()=>toggleTooltipHandlers(self)
    ...
    dispose() {
        ...
        } else disposeTooltipComplete(self);
...
else document.addEventListener('DOMContentLoaded', ()=>initCallback()
, {
    once: true
});
var index = {
...
//# sourceMappingURL=index.5c65a923.js.map
